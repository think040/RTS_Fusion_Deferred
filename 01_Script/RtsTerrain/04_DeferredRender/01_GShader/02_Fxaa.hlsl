...
PS_Out PShader(RS_Out pin)
{
    ...
    float4 diffuse = finalTex.Sample(sampler_finalTex, uv);
    
    float4 color;
    
    {
        uint dx, dy;
        finalTex.GetDimensions(dx, dy);
        float2 rcpro = rcp(float2(dx, dy));
    
        FxaaTex t;
        t.smpl = sampler_finalTex;
        t.tex = finalTex;
    
        
        
        //color = FxaaPixelShader(uv, 0, t, t, t, rcpro, 0, 0, 0, 1.0, 0.166, 0.0312, 0, 0, 0, 0);
        //color = FxaaPixelShader(uv, 0, t, t, t, rcpro, 0, 0, 0, 0.0f, 0.5f, 0.05f, 0, 0, 0, 0);
        //color = FxaaPixelShader(uv, 0, t, t, t, rcpro, 0, 0, 0, 1.0, 1.0f, 0.1f, 0, 0, 0, 0);
        color = FxaaPixelShader(uv, 0, t, t, t, rcpro, 0, 0, 0, fxaa_data.x, fxaa_data.y, fxaa_data.z, 0, 0, 0, 0);        
    }
